...
#
# in case no wipe, touching WorkflowApp and main si get a new build
#

touch WorkflowAppHydroUQ.cpp main.cpp
...
rm -fr "${APP_DIR}/Contents/MacOS/Examples/.conf"
rm -fr "${APP_DIR}/Contents/MacOS//Examples/hdro-0021/results"
...
if [ "${DMG_METHOD}" = "NEW" ]; then
    ...
    (
        ...
        ../macInstall/create-dmg \
            ...
            "Hydro-UQ_Mac_Download_${ARCH}.dmg" \
            "app"
    ...
    (
        cd "${BUILD_DIR}"
        hdiutil create "Hydro-UQ_Mac_Download_${ARCH}.dmg" -fs HFS+ -srcfolder "./${APP_NAME}" -format UDZO -volname "${APP_NAME}"
    )
...
echo "xcrun notarytool submit ${BUILD_DIR}/Hydro-UQ_Mac_Download_${ARCH}.dmg --apple-id $appleID --password $appleAppPassword --team-id $appleCredential"
xcrun notarytool submit "${BUILD_DIR}/Hydro-UQ_Mac_Download_${ARCH}.dmg" --apple-id "$appleID" --password "$appleAppPassword" --team-id "$appleCredential"
...
echo "xcrun stapler staple  ${BUILD_DIR}/Hydro-UQ_Mac_Download_${ARCH}.dmg"
