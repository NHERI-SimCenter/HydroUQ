...
One of the primary inputs from GeoClaw includes the bathymetric data. The topography is assumed as a value of :math:`z` (relative to a particular reference level) at each point :math:`(x,y)`. Most often the water surface is considered as :math:`z=0` for reference. In addition, GeoClaw recognizes only uniformly spaced rectangular topography grids.
...
The first format, known as *topotype = 1* stores the :math:`(x,y,z)` points of all the points in the domain. The size and spacing of the grid is thus obtained from the data. The files is generally as below
