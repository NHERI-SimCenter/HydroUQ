...
const int layerColorThickness(64 - 4); // 60
const int heightToColorDiv(128 + 12 + 240 / 6);
...
const int aboveWaterGroundColorsMax(aboveWaterGroundColorsMin + 1 * layerColorThickness);
const int underWaterGroundColorsMin(aboveWaterGroundColorsMax + 1);
const int underWaterGroundColorsMax(underWaterGroundColorsMin + 1 * layerColorThickness);
...
void VolumetricModifier::adjustAlphaMultiplier(int value)
{
    if (m_volumeItem) {
        ...
        //! [11]
        QString labelFormat = QStringLiteral("Opacity multiplier: %1");
...
void VolumetricModifier::toggleAreaAll(bool enabled)
{
    if (enabled) {
        ...
        m_graph->axisY()->setRange(-20.0f, 10.0f);
        m_graph->axisZ()->setRange(0.0f, 1000.0f);
        m_graph->axisX()->setSegmentCount(12);
        m_graph->axisY()->setSegmentCount(12);
        m_graph->axisZ()->setSegmentCount(12);
    }
}
...
void VolumetricModifier::toggleAreaMine(bool enabled)
{
    if (enabled) {
        ...
        m_graph->axisY()->setRange(-7.5f, 7.5f);
        m_graph->axisZ()->setRange(350.0f, 850.0f);
        m_graph->axisX()->setSegmentCount(12);
        m_graph->axisY()->setSegmentCount(12);
        m_graph->axisZ()->setSegmentCount(12);
    }
...
{
    if (enabled) {
        m_graph->axisX()->setRange(100.0f, 600.0f);
                                     ...
                                     QVector<uchar> *textureData)
{
    ...
    float yMin = m_graph->axisY()->min();
    float yMax = m_graph->axisY()->max();

    int seaLevel = int(5.0f / (yMax - yMin) * (240.0f + 4.f + 30.f + 12.f));
    ...
    for (int i = startIndex; i < endIndex; i++) {
        ...
        for (int l = 0; l < textureSize; l++) {
            ...
            waterHeights[l] = int(m_waterLayer.at(layerIndex)) + seaLevel;
...
void VolumetricModifier::initMineShaftArray()
{
    ...
    float shifty = -0.025f;
    float shiftz = 0.075f;
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.2f, 0.7f + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.9f, 0.7f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.8f, 0.5f + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.8f, 0.7f + shiftz));

    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.4f  + shiftx,  0.2f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.7f  + shiftx,  0.2f + shifty, 0.7f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.4f  + shiftx,  0.2f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.4f  + shiftx,  0.2f + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.45f + shiftx, 0.2f  + shifty, 0.7f + shiftz),
                                                    QVector3D(0.45f + shiftx, 0.2f  + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.5f  + shiftx,  0.2f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.5f  + shiftx,  0.2f + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.55f + shiftx, 0.2f  + shifty, 0.7f + shiftz),
                                                    QVector3D(0.55f + shiftx, 0.2f  + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f  + shiftx,  0.2f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.6f  + shiftx,  0.2f + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.65f + shiftx, 0.2f  + shifty, 0.7f + shiftz),
                                                    QVector3D(0.65f + shiftx, 0.2f  + shifty, 0.8f + shiftz));

    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.5f  + shiftx,  0.3f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.7f  + shiftx,  0.3f + shifty, 0.7f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.5f  + shiftx,  0.3f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.5f  + shiftx,  0.3f + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.55f + shiftx, 0.3f  + shifty, 0.7f + shiftz),
                                                    QVector3D(0.55f + shiftx, 0.3f  + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f  + shiftx,  0.3f + shifty, 0.7f + shiftz),
                                                    QVector3D(0.6f  + shiftx,  0.3f + shifty, 0.8f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.65f + shiftx, 0.3f  + shifty, 0.7f + shiftz),
                                                    QVector3D(0.65f + shiftx, 0.3f  + shifty, 0.8f + shiftz));

    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.2f + shifty, 0.4f  + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.2f + shifty, 0.7f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.45f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.45f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.5f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.5f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.55f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.55f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.6f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.6f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.65f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.65f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.7f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.7f  + shiftz));

    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.3f + shifty, 0.4f  + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.3f + shifty, 0.7f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.45f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.45f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.5f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.5f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.55f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.55f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.6f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.6f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.65f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.65f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.3f + shifty, 0.7f  + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.3f + shifty, 0.7f  + shiftz));

    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.2f + shifty, 0.5f  + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.2f + shifty, 0.7f  + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.55f + shiftz),
                                                    QVector3D(0.8f + shiftx, 0.2f + shifty, 0.55f + shiftz));
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.6f + shiftx, 0.2f + shifty, 0.6f  + shiftz),
    ...
    m_mineShaftArray << QPair<QVector3D, QVector3D>(QVector3D(0.7f + shiftx, 0.2f, 0.4f + shiftz),
                                                    QVector3D(0.7f + shiftx, 0.8f, 0.4f + shiftz));
}
